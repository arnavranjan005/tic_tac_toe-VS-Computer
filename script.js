let firstRow=document.getElementById("f").getElementsByTagName("span");
    ...
    let matrix=[[firstRow[0],firstRow[1],firstRow[2]],
    ...
    let Computer=(row,col,counter,maxi,tie)=>{
        ...
        for (let i = 0; i < matrix.length; i++) {
            for (let j = 0; j < matrix[i].length; j++) {
                ...
                let maxiO=Math.max(mpCO[j],mpRO[i],mpRaddCO[i+j],mpRsubCO[2+j-i]);
                if(((maxiO==mpCO[j])||(maxiO==mpRO[i])||maxiO==mpRaddCO[i+j]||maxiO==mpRsubCO[2+j-i])
                    && maxiO==2 
        ...
        if((maxi==mpC[col])&&((i!=row&&j==col)&&(matrix[i][j].textContent==""))){
                mpCO[j]++;
                mpRO[i]++;
                mpRaddCO[i+j]++;
                mpRsubCO[2+j-i]++;
        ...
        else if((maxi==mpR[row])&&((i==row&&j!=col)&&(matrix[i][j].textContent==""))){
            mpCO[j]++;
            mpRO[i]++;
            mpRaddCO[i+j]++;
            mpRsubCO[2+j-i]++;
        ...
        else if((maxi==mpRaddC[row+col])&&((j+i==row+col)&&(matrix[i][j].textContent==""))) {
            mpCO[j]++;
            mpRO[i]++;
            mpRaddCO[i+j]++;
            mpRsubCO[2+j-i]++;
        ...
        else if((maxi==mpRsubC[2+col-row])&&((2+j-i==2+row-col)&&(matrix[i][j].textContent==""))) {
            mpCO[j]++;
            mpRO[i]++;
            mpRaddCO[i+j]++;
            mpRsubCO[2+j-i]++;    
        ...
        if(counter<500)
        Computer(row,col,counter,maxi,tie);
        else{
            if((maxi==mpC[col]) &&(maxi!=mpR[row])) maxi=mpR[row];
            else if((maxi==mpR[row])&&(maxi!=mpRaddC[row+col])) maxi=mpRaddC[row+col];
            else if((maxi==mpRaddC[row+col])&&(maxi!=mpRsubC[2+row-col])) maxi=mpRsubC[2+row-col];
            else if((maxi==mpRsubC[2+row-col])&&(maxi!=mpC[col])) maxi=mpC[col];
            tie++;
    ...
    matrix.forEach(i=>{
        i.forEach(j=>{
            j.addEventListener("click",()=>{
                ...
                if(mpR[row]==3||mpC[col]==3||mpRaddC[row+col]==3||mpRsubC[2+row-col]==3){
                    document.getElementById("win").style.display="inline-block";
                    document.getElementById("win").childNodes[2].textContent+= countmove.toString();
                    win[0].style.display="flex";
                return;}
                let counter=0;
                let maxi= Math.max(mpC[col],mpR[row],mpRaddC[row+col],mpRsubC[2+col-row])
                Computer(row,col,counter,maxi,0);
    })})})

    document.getElementById("tie").addEventListener("click",()=>{window.location.reload();})

    document.getElementById("win").addEventListener("click",()=>{window.location.reload();})

    document.getElementById("lose").addEventListener("click",()=>{window.location.reload();})
